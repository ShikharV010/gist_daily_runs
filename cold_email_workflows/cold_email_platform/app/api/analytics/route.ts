...
// Four metrics, attributed by call_at date in the requested timezone:
//  - calls within 5 min of reply
//  - calls outside 5 min
//  - bookings (Meeting Booked) from <5 min calls
//  - bookings from >=5 min calls
...
export async function GET(req: NextRequest) {
  ...
  const totalsRows = await query<{
    ...
    `SELECT
       ...
       COUNT(*) FILTER (WHERE row_type = 'dialer' AND call_within_5min = false)                                             AS calls_outside_5min,
       COUNT(*) FILTER (WHERE row_type = 'dialer' AND call_disposition ILIKE '%meeting booked%' AND call_within_5min = true)  AS bookings_within_5min,
       COUNT(*) FILTER (WHERE row_type = 'dialer' AND call_disposition ILIKE '%meeting booked%' AND call_within_5min = false) AS bookings_outside_5min
